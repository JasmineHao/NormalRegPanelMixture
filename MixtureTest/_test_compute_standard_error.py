...
rho = np.array([0.5, 0.7])    # AR(1) coefficients
mu = np.array([-1.0, 1.0])    # Mean values for each subcategory (M x K)
sigma = np.array([0.3, 0.3])  # Standard deviation for each category (length M)
beta = np.array([[0.0, 1.0], [1.0, 0.0]])  # Coefficients for q covariates (M x q)
gamma = np.array([0.5])  # Coefficients for p covariates (length p)
N = 200  # Number of individuals
T = 5    # Number of time periods
M = 2    # Number of categories
p = 1    # Number of covariates in z
...
for iteration in range(100):
    ...
    # Generate data for AR(1) normal error model
    ...
    # Compute the score and Hessian for AR(1) normal model
...
# Print the table
print(results_df)
print("AR(1) Normal Error Model Confidence Interval Coverage:", confidence_interval_coverage)


# for key in params_dict.keys():
#     print(f"Field: {key}")
#     print(f"From params_dict: {params_dict[key]}")
#     print(f"From params_dict_from_array: {params_dict_from_array[key]}")
#     print("-" * 50)
...
# Test case: AR(1) normal error model without covariates
# ------------------------------------------------------

# Update parameters for no covariates
...
for iteration in range(100):
    ...
    # Generate data for AR(1) normal error model without covariates
    data = generate_data_ar1(alpha, rho, mu, sigma, beta, gamma, mu_0, sigma_0, beta_0, gamma_0, N, T, M, p, q)
    y, x, z = data
...
# Print results for AR(1) normal error model without covariates
results_df = pd.DataFrame({
    "Parameter Name": parameter_names,
    "True Parameter": true_parameters,
    "Standard Error": ar1_standard_errors,
...
# Print the table
print(results_df)
print("AR(1) Normal Error Model Without Covariates Confidence Interval Coverage:", confidence_interval_coverage)
# %%
