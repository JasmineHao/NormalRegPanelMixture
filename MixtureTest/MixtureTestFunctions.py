...
def score_ar1_mixture(data, params_dict):# -> tuple[NDArray, NDArray]:
    y, x, z = data
    t, n = y.shape
    q = x.shape[1]
    p = z.shape[1]
...
def get_params_ar1_normal(out_h0):
    """
    Extracts parameter estimates from the out_h0 object and returns them as a dictionary
    and a flattened array.

    Parameters:
    - out_h0: Dictionary containing parameter estimates.
    - q: Number of covariates in x.
    - M: Number of categories.

    Returns:
    - params_dict: Dictionary containing parameter estimates.
    - params_array: Flattened array of parameter estimates.
    """
    # Extract parameters
    alpha_hat = out_h0['alpha_hat'][0]
    m = len(alpha_hat)
    ...
    p = len(gamma_hat)
    # Reshape mubeta_hat and mubeta_0_hat
    mubeta_hat = np.ascontiguousarray(mubeta_hat)
    mubeta_hat_mat = mubeta_hat.reshape((q + 1, m)).T
    beta_hat = mubeta_hat_mat[:, 1:].T.flatten()

    mubeta_0_hat = np.ascontiguousarray(mubeta_0_hat)
    mubeta_0_hat_mat = mubeta_0_hat.reshape((q + 1, m)).T
    beta_0_hat = mubeta_0_hat_mat[:, 1:].T.flatten()
    mu_0_hat = mubeta_0_hat_mat[:, 0]
...
def score_i_ar1_normal(y_it, x_it, z_it, params_array, m, k, q, p):
    ...
    w = np.zeros(m)
    for j in range(m):
        residuals = (y_it[1:] - x_it[1:] @ beta_mat[j] - z_it[1:] @ gamma - mu[j]) - rho[j] * (y_it[:-1] - x_it[:-1] @ beta_mat[j] - z_it[:-1] @ gamma - mu[j])
        residuals_0 = y_it[0] - x_it[0] @ beta_0_mat[j] - z_it[0] @ gamma_0 - mu_0[j]
    ...
    for j in range(m):
        ...
        for qq in range(q):
            beta_id = j + m * qq
            score_beta[beta_id] += w[j] * np.sum(residuals * (x_it[1:, qq] - rho[j] * x_it[:-1, qq])) / sigma[j] ** 2
            score_beta_0[beta_id] += w[j] * residuals_0 * x_it[0, qq] / sigma_0[j] ** 2
        ...
        score_mu[j] = w[j] * np.sum(residuals) / sigma[j] ** 2
        score_mu_0[j] = w[j] * residuals_0 / sigma_0[j] ** 2

        score_rho[j] += w[j] * np.sum(residuals * (y_it[:-1] - x_it[:-1] @ beta_mat[j] - z_it[:-1] @ gamma)) / sigma[j] ** 2
...
def score_ar1_normal(data, params_dict):
    ...
    for i in range(n):
        ...
        scores.append(score_i)  # Append the score for the current observation
        hessians.append(hessian_i)
    # Ensure consistency with score_ar1_normal by summing scores across observations
    score = np.vstack(scores)  # Concatenate all scores into a single array
    hessian = np.array(hessians)
    return score, hessian
