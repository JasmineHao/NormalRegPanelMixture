...
def log_likelihood_array(y, mu, sigma):
    ...
    variance = max(sigma ** 2, 1e-10)  # Avoid division by zero
...
def EM_optimization(y_c, x, z, p, q, sigma_0, alpha_draw, mubeta_draw, sigma_draw, gamma_draw, m, t, an, maxit=2000, tol=1e-8, tau = 0.5, epsilon=0.01, alpha_bound = 0.05):
    ...
    for jn in range(ninits):
        ...
        for iter_ii in range(maxit):  # iter_ii is used as a loop counter
    ...
    for jn in prange(ninits):    
        ...
        for iter_ii in range(maxit):           
            ...
            for j in range(m):
                ...
                mubeta_jn_mat[j,0] = min(2.0, max(-2.0, mubeta_estim[0] * (1 - rho_estim)))
                ...
                mubeta_0_jn_mat[j,0] = min(2.0, max(-2.0, mubeta_0_jn_mat[j,0]))
        ...
        for iter_ii in range(maxit):
            ...
            for mm in range(m):
                ...
                for kk in range(k):
                    ...
                    mu_jn[idx_type] = min(2.0, max(-2.0, mu_jn[idx_type]))
                    
                    mu_0_jn[idx_type] = np.mean(ytilde_0 * w_mk_0[idx_type,:]) / max(np.mean(w_mk_0[idx_type,:]), 1e-6)
                    mu_0_jn[idx_type] = min(2.0, max(-2.0, mu_0_jn[idx_type]))
...
def EM_optimization_AR1_noconstraint(y_c, xz, y_l, x_c, x_l, z_c, z_l, y_0, xz_0, p, q,  sigma_0, alpha_draw,  mubeta_draw, sigma_draw, gamma_draw, mubeta_0_draw, sigma_0_draw, gamma_0_draw, m, n, t, an, maxit=2000, tol=1e-8, epsilon=0.01, alpha_bound = 0.05):
    ...
    x1 = np.hstack((np.ones((nt, 1)), x_c, y_l[:,np.newaxis]))
    z1 = z_c
    ...
    for jn in prange(ninits):    
        ...
        for iter_ii in range(maxit):           
            ...
            for j in range(m):
                ...
                mubeta_estim = solve_linear_system_safe(xtilde.T @ x1, xtilde.T @ (y_c - z1 @ gamma_jn))
            ...
            if p > 0:
                ...
                for j in range(m):
                    ...
                    ze += (ztilde.T @ (y_c - x1 @ mubeta_jn_mat[j,:])).reshape((p, 1)) / max(sigma_jn[j]**2, 0.01)
                gamma_jn = solve_linear_system_safe(zz,ze).flatten()
...
def EM_optimization_AR1_mixture_noconstraint(y_c, xz, y_l, x_c, x_l, z_c, z_l, y_0, xz_0, p, q,  sigma_0, alpha_draw, tau_draw, mubeta_draw, sigma_draw, gamma_draw,
    ...
    for jn in prange(ninits):
        ...
        for iter_ii in range(maxit):
            ...
            for mm in range(m):
                for kk in range(k):
                    res = y_c - x1 @ mubeta_jn_mat[mm] - z_c @ gamma_jn - mu_jn[mm*k + kk]
                    res_0 = y_0 - x1_0 @ mubeta_0_jn_mat[mm] - z_0 @ gamma_0_jn - mu_0_jn[mm*k + kk]
                    r[mm*k + kk] = log_likelihood_array(res, 0.0, sigma_jn[mm])
                    r_0[mm*k + kk] = log_likelihood_array(res_0, 0.0, sigma_0_jn[mm])
            ...
            for mm in range(m):
                ...
                xtilde = np.zeros((nt, q1-1))
                for ii in range(q1-1):
                    xtilde[:, ii] = w_m * x1[:, ii+1]

                y_tmp = y_c - mu_mk_weighted[mm] - z_c @ gamma_jn
                
                mubeta_estim = solve_linear_system_safe(xtilde.T @ x1[:,1:], xtilde.T @ y_tmp)
                mubeta_jn_mat[mm,1:] = mubeta_estim
                
                # estimate mubeta_0 for initial period
                xtilde_0 = np.zeros((n, x1_0.shape[1]-1))
                for ii in range(x1_0.shape[1]-1):
                    xtilde_0[:, ii] = wtilde * x1_0[:, ii+1]

                coef_0_jn = solve_linear_system_safe(xtilde_0.T @ x1_0[:,1:], xtilde_0.T @ (y_0 - z_0 @ gamma_0_jn - mu_mk_0_weighted[mm]))
                
                mubeta_0_jn_mat[mm,1:(q+1)] = coef_0_jn

                ytilde = y_c - x1[:,1:] @ mubeta_jn_mat[mm,1:] - z_c @ gamma_jn               
                ytilde_0 = y_0 - z_0 @ gamma_0_jn - x1_0 @ mubeta_0_jn_mat[mm]
            ...
            if p > 0:
                ...
                for mm in range(m):
                    ...
                    for ii in range(p):
                        ztilde[:, ii] = w_j * (z_c[:,ii])
                    for ii in range(p):
                        ztilde_0[:, ii] = wtilde * z_0[:, ii]
                        
                    zz += (ztilde.T @ z_c) / max(sigma_jn[mm]**2, 1e-6)
                    ze += (ztilde.T @ (y_c - x1 @ mubeta_jn_mat[mm,:] - mu_mk_weighted[mm]).reshape(-1, 1)) / max(sigma_jn[mm]**2, 1e-6)
                    
                    zz_0 += ztilde_0.T @ z_0 / max(sigma_0_jn[mm]**2, 1e-6)
...
def regpanelmixmixtureAR1NoConstraintPMLE(y, x, z, p, q, m, k, ninits=10, tol_long=1e-6, maxit=2000, tol_short=1e-2, maxit_short=200, alpha_bound=0.05, tau_bound = 0.05): 
    ...
    for mm in range(m):
        ...
        for kk in range(k):
            idx_type = mm * k + kk
            # Draw mu in the interval between minMU and maxMU for idx_type
            lb = minMU + (maxMU - minMU) * quantile_cutoff[kk]
            ub = minMU + (maxMU - minMU) * quantile_cutoff[kk+1]
            mu_draw[idx_type, :] = np.random.uniform(lb, ub, size=ninits_short)
            # Draw mu_0 in the interval between minMU_0 and maxMU_0 for idx_type
            lb_0 = minMU_0 + (maxMU_0 - minMU_0) * quantile_cutoff[kk]
            ub_0 = minMU_0 + (maxMU_0 - minMU_0) * quantile_cutoff[kk+1]
            mu_0_draw[idx_type, :] = np.random.uniform(lb_0, ub_0, size=ninits_short)
    ...
    mubeta_hat = mubeta_mat.T.flatten() 
    mubeta_0_mat =  np.ascontiguousarray(mubeta_0_hat).reshape((q+1,m)).T
    
    mubeta_0_hat = mubeta_0_mat[:,:(q+1)].T.flatten() 
...
def LRTestNormal(y, x, z, p, q, m, N, T, bootstrap = True, BB= 199, spline=False, alpha_bound=0.05, epsilon=0.05):
    ...
    if bootstrap:
        # Generate data for bootstrap
        Data = [generate_data(alpha_hat, mu_hat, sigma_hat, gamma_hat, beta_hat, N, T, m, p, q, spline=spline, x_input=x, z_input=z) for _ in prange(BB)]
...
def LRTestMixture(y, x, z, p, q, m, k, N, T, bootstrap = True, BB= 199, spline=False, alpha_bound=0.05, tau_bound=0.05, epsilon=0.05):
    ...
    if bootstrap:
        # Generate data for bootstrap
        Data = [generate_data_mixture(alpha_hat, mu_hat, beta_hat, sigma_hat, tau_hat, gamma_hat, N, T, m, k, p, q, spline=spline, x_input=x, z_input=z) for _ in prange(BB)]
...
def LRTestAR1Mixture(y, x, z, p, q, m, k, N, T, bootstrap = True, BB= 199, spline=False, alpha_bound=0.05, tau_bound=0.05):
    ...
    if bootstrap:
        # Generate data for bootstrap
        Data = [generate_data_ar1_mixture(alpha_hat, tau_hat, rho_hat, mu_hat, sigma_hat, beta_hat, gamma_hat, mu_0_hat, sigma_0_hat, beta_0_hat, gamma_0_hat, N, T, m, k, p, q, spline=spline, x_input=x, z_input=z) for _ in prange(BB)]
...
# %%
@njit(parallel=False)
def LRTestAR1Normal(y, x, z, p, q, m, N, T, bootstrap = True, BB= 199, spline=False, alpha_bound=0.05):
    out_h0 = regpanelmixAR1PMLE(y, x, z, p, q, m, alpha_bound=alpha_bound)
    out_h1 = regpanelmixAR1PMLE(y, x, z, p, q, m+1, alpha_bound=alpha_bound)
    alpha_hat  = out_h0['alpha_hat'][0]
    mubeta_hat = out_h0['mubeta_hat'][0]
    ...
    if bootstrap:
        # Generate data for bootstrap
        Data = [generate_data_ar1(alpha_hat, rho_hat, mu_hat, sigma_hat, beta_hat, gamma_hat, mu_0_hat, sigma_0_hat, beta_0_hat, gamma_0_hat, N, T, m, p, q, x_input=x, z_input=z) for _ in prange(BB)]
...
# %%     
@njit(parallel=False)
def LRTestAR1NormalNoConstraint(y, x, z, p, q, m, N, T, bootstrap = True, BB= 199, spline=False, alpha_bound=0.05):
    ...
    mubeta_hat = np.ascontiguousarray(mubeta_hat)
    mubeta_hat_mat = mubeta_hat.reshape((q+2),m).T
    beta_hat = mubeta_hat_mat[:,1:]
    mu_hat =  mubeta_hat_mat[:,0]
    ...
    if bootstrap:
        # Generate data for bootstrap
        Data = [generate_data_ar1_noconstraint(alpha_hat, mu_hat, sigma_hat, beta_hat, gamma_hat, mu_0_hat, sigma_0_hat, beta_0_hat, gamma_0_hat, N, T, m, p, q, x_input=x, z_input=z) for _ in prange(BB)]
...
def LRTestAR1MixtureNoConstraint(y, x, z, p, q, m, k, N, T, bootstrap = True, BB= 199, spline=False, alpha_bound=0.05):
    ...
    if bootstrap:
        # Generate data for bootstrap
        Data = [generate_data_ar1_mixture_noconstraint(alpha_hat, tau_hat, mu_hat, sigma_hat, beta_hat, gamma_hat, mu_0_hat, sigma_0_hat, beta_0_hat, gamma_0_hat, N, T, m, k, p, q, x_input=x, z_input=z) for _ in prange(BB)]
...
# %%
def process_model(model_function, y, x, z, p, q, m, k, model_type, specification, output_prefix, INDNAME, if_bootstrap=True):
        ...
        if if_bootstrap:
            ...
            params_dict, params_array, standard_errors, standard_errors_dict, variable_names = compute_standard_errors(model_output, data=[y, x, z], model_type=model_type)
...
def restore_estimated_parameters_from_table(csv_path):
    # csv_path = f"estimated_parameters_table_bootstrap_m{figure_m}_0526.csv"
