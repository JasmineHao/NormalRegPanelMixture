                            ...
                            cl = NULL, nrep = 1000, ninits.crit = 25, deep.check=0)
{
          ...
          if (q > 1){
            ...
            for (jj in 1:(q-1)) {
              for (ii in (jj+1):q) {
                ...
                s_lam_7[nn,qq] = w_m_T[nn,j] * (2 * H_2_n %*% (xx_i * xx_j) + sum(tmp[row(tmp)!=col(tmp)]) )
                qq <- qq+1
              }
            }
          }
  ...
  e_val <- sqrt(e$val)
  e_val[is.na(e_val)] <- 1e-16
  u <- t(e$vec %*% (t(e$vec) * e_val) %*% matrix(rnorm(nrep*n_lam*m), nrow=n_lam*m))
...
regpanelmixCritBoot <- function (y, x, parlist, z = NULL, values = NULL, ninits = 100, nbtsp = 199, parallel = FALSE, an = 0.5, cl = NULL, eps=0.01) {
  ...
  ybset <- replicate(nbtsp, generateData(N = n, T = t,M=m ,alpha = alpha, mu = mu, beta = beta, sigma = sigma, gam=gam, p= p,q=q, x= x, z = z))
...
regpanelmixCritBootAR1 <- function (y, x, parlist, z = NULL, values = NULL, ninits = 100, nbtsp = 199, parallel = FALSE, an = 0.5, an_0 = 0.5,  cl = NULL, data.0 = NULL)
{
  ...
  if (m == 1){
    beta <- t(matrix(cbind(rho, beta.r, -rho * beta.r),nrow = q))
  }else{
  ...
  if (q.eff > 0){
    beta0 <- (beta.r / sqrt(abs(sigma**2 / (1 - rho**2))))
  } else{
    beta0 <- NULL 
  }
