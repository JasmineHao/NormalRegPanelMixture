...
// Sigmoid function
inline arma::vec sigmoid(const arma::vec& z) {
...
arma::vec logistic_regression(const arma::mat& X, const arma::vec& y, double alpha, int iterations) {
  ...
  arma::vec theta = arma::zeros(n);
  
  for(int i = 0; i < iterations; i++) {
    arma::vec z = X * theta;
    arma::vec h = sigmoid(z);
    arma::vec gradient = X.t() * (h - y) / m;
...
List cppRegPanelmixPMLEAR1(NumericMatrix bs,
  ...
  arma::mat zinit(zinits.begin(), zinits.nrow(), zinits.ncol(), false);
  ...
  arma::mat gammainit(q10, m);
  arma::vec gammainittemp(q10);
  ...
  arma::mat wi(m,n); //Weight matrix, to be used for update gammainit
  arma::mat pi(m,n); //Initial weight matrix
  ...
  for (int jn=0; jn<ninits; jn++) {
    ...
    {
      for (int i = 0; i < q10; ++i){
        gammainit(i, j) = b_jn((q1 + 2 ) * m + p + i + j * q10  );
      }
    }
    gammainit.col(m-1) = arma::zeros(q10);
    
    // 
    // first initialize the pi probability
    
    for (int nn = 0; nn < n; nn++){
      pi.col(nn) = exp(zinit.row(nn) * gammainit) / sum(exp(zinit.row(nn) * gammainit));
    }
    ...
    /* update gammainit */
    for (int j = 0; j < m-1; j++){
      gammainittemp = logistic_regression(zinit,wi.row(j),0.01,1000);
      if (arma::all(gammainittemp < 100)){
        gammainit.col(j) = gammainittemp;
      }
    }
