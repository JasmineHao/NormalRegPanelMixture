...
for (N in Nset){
  ...
  for (mu in muset){
    ...
    results <- foreach (ii = 1:nrep, .packages = c("expm", "Matrix", "NormalRegPanelMixture", "MASS"))%dopar% {
      ...
      data <- Data[, ii]
      result_rk <- compute_rk_statistics_triplet_T(data, T.triplet.list, N, M,  n.grid = 3)
        #compute_rk_statistics_pairwise_T(data, T.pair.list, N, M,   n.grid = 3)
    ...
    for (ii in 1:nrep) {
      ...
      lr_stat.crit[ii] <- results[[ii]]$lr.crit.m[2]
