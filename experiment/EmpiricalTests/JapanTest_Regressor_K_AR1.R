...
for (each.code in ind.code){
  ...
  for (T in 3:3){
    ...
    for (M in 1:5){
      ...
      out.h1 <- regpanelmixMaxPhi(y = data$Y, x = data$X, z = data$Z, parlist = out.h0$parlist, an = an, an_0 = an_0, data.0 = data.0, ninits=2)
      ...
      if (estimate.crit) {
        t <- Sys.time()
        lr.crit <- regpanelmixCritBootAR1(y = data$Y, x = data$X, parlist = out.h0$parlist, z = data$Z, cl = cl, parallel = TRUE, data.0 = data.0, an = an, an_0 = an_0, ninits=10,nbtsp = 199)$crit
        print(Sys.time() - t)
  ...
  sink(paste("results/Empirical/Japan_Crit_", ind.name, "_K_AR1.txt"))
