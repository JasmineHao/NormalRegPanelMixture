...
for (each.code in ind.code){
  ...
  data <- list(
    Y = t(ind.each.y), 
    X = data.frame(ind.each.t[,c("y_l1", "lnk","lnl","lnk_l1","lnl_l1")]), 
    Z = ind.each.ciiu_dummy)
  
  data.0 <- list(
    Y = ind.each.t[ind.each.t$year==t.start,]$y_l1,
    X = data.frame(ind.each.t[ind.each.t$year==t.start,c("lnk_l1","lnl_l1")]), 
    Z = ind.each.ciiu_dummy[ind.each.t$year==t.start,]) # for the initial condition
  
  
  # data <- list(Y = t(ind.each.y), X = ind.each.t[, c(bs_columns, c("import","export"))]  ,  Z = ind.each.ciiu_dummy)
  N <- dim(ind.each.y)[1]
  ...
  for (M in 1:10){
    ...
    out.h1 <- regpanelmixMaxPhi(y=data$Y,x=data$X, z = data$Z,parlist=out.h0$parlist,an=an,ninits = 1)
    ...
    if (estimate.crit == 1){
      lr.crit <- regpanelmixCritBootAR1(y = data$Y, x = data$X, parlist = out.h0$parlist, z = data$Z, cl = cl, parallel = TRUE, data.0 = data.0, an = an, an_0 = 0)$crit
    }
...
write.csv(df.3,file=paste("results/Empirical/Additional/Chile_regressior_", model,".csv",sep=""))
