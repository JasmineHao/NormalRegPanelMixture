...
getEstimateDiffAn <- function(Data,nrep,an,cl,M, parlist){
  ...
  results <- foreach (k = 1:nrep)%dopar% {
    ...
    out.h1.m <- NormalRegPanelMixture::normalpanelmixMaxPhi(y=data$Y,parlist=out.h0$parlist,an=0,parallel = FALSE)
...
write.csv(rbind(result.h,result.m,result.l), file="results/sizeTestM2Sim_no_penalty.csv")
