...
getEstimateDiffAn <- function(Data,nrep,an,cl,M){
  ...
  results <- foreach (k = 1:nrep)%dopar% {
    ...
    out.h1.m <- NormalRegPanelMixture::regpanelmixMaxPhi(y=data$Y,x=data$X, z = data$Z,parlist=out.h0$parlist,an=(an),parallel = FALSE)
    out.h1.l <- NormalRegPanelMixture::regpanelmixMaxPhi(y = data$Y, x=data$X, z = data$Z, parlist = out.h0$parlist, an = ( 0.01 * an), parallel = FALSE)
    out.h1.h <- NormalRegPanelMixture::regpanelmixMaxPhi(y = data$Y, x=data$X, z = data$Z, parlist = out.h0$parlist, an = (100 * an), parallel = FALSE)
  ...
  return(list(crit = lr.crit.m,nominal.size.l = apply(lr.size.l,2,mean),nominal.size.m = apply(lr.size.m,2,mean) , nominal.size.h = apply(lr.size.h,2,mean) ))
}
...
write.csv(rbind(result.h,result.m,result.l), file="results/sizeTestM2RegressorBoot_HML.csv")
write.csv(result.h, file = "results/sizeTestM2RegressorBootH_1.csv")
write.csv(result.m, file = "results/sizeTestM2RegressorBootM_1.csv")
write.csv(result.l, file = "results/sizeTestM2RegressorBootL_1.csv")
