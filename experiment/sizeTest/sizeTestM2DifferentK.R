...
getEstimateDiffAn <- function(Data,nrep,an,cl,M, parlist, k_max){
  ...
  results <- foreach (k = 1:nrep)%dopar% {
    ...
    out.h1.l <- NormalRegPanelMixture::normalpanelmixMaxPhi(y=data$Y,parlist=out.h0$parlist,an=(0.1 * an),parallel = FALSE, k_max = k_max)
    out.h1.m <- NormalRegPanelMixture::normalpanelmixMaxPhi(y=data$Y,parlist=out.h0$parlist,an=(an),parallel = FALSE, k_max = k_max)
    out.h1.h <- NormalRegPanelMixture::normalpanelmixMaxPhi(y=data$Y,parlist=out.h0$parlist,an=(10 * an) ,parallel = FALSE, k_max = k_max)
  ...
  return(list(crit = lr.crit,nominal.size.l = apply(lr.size.l,2,mean),
              nominal.size.m = apply(lr.size.m,2,mean) , 
              nominal.size.h = apply(lr.size.h,2,mean), 
              lr.estimate.l = lr.estimate.l, lr.estimate.m = lr.estimate.m, lr.estimate.h = lr.estimate.h ))
}
...
for (r in 1:nNT){
  ...
  for (count in 1:nPar){
    ...
    result <- getEstimateDiffAn(Data,nrep,an,cl,M, parlist, k_max)
...
write.csv(result.h, file = paste("results/sizeTestM2SimH_K", k_max ,".csv", sep="_"))
write.csv(result.m, file = paste("results/sizeTestM2SimM_K", k_max ,".csv", sep="_"))
write.csv(result.l, file = paste("results/sizeTestM2SimL_K", k_max ,".csv", sep="_"))
