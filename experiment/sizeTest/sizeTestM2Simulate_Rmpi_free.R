...
getEstimateDiffAn <- function(Data,nrep,an,cl){
  ...
  for (k in 1:nrep){
    ...
    out.h1.m <- normalpanelmixMaxPhi(y=data$Y,parlist=out.h0$parlist,an=(an),update.alpha = 1,parallel = TRUE,cl=cl)
    ...
    lr.estimate.m[k,] <- 2 * max(out.h1.m$penloglik - out.h0$loglik)
...
write.csv(result.h,file="experiment/sizeTest/sizeTestM2SimH.csv")
write.csv(result.m,file="experiment/sizeTest/sizeTestM2SimM.csv")
write.csv(result.l,file="experiment/sizeTest/sizeTestM2SimL.csv")
