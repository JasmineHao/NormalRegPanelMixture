...
getEstimateDiffAn <- function(Data,nrep,an,cl,M){
  ...
  results <- foreach (k = 1:nrep)%dopar% {
    ...
    out.h1.l <- NormalRegPanelMixture::regpanelmixMaxPhi(y=data$Y,x=data$X, parlist=out.h0$parlist,an=0,parallel = FALSE, eps=0.01)
    out.h1.m <- NormalRegPanelMixture::regpanelmixMaxPhi(y=data$Y,x=data$X, parlist=out.h0$parlist,an=0,parallel = FALSE, eps=0.05)
    out.h1.h <- NormalRegPanelMixture::regpanelmixMaxPhi(y=data$Y,x=data$X, parlist=out.h0$parlist,an=0,parallel = FALSE, eps=0.5)
    
    
    crit.m <- NormalRegPanelMixture::regpanelmixCritBoot(y=data$Y, x=data$X, parlist=out.h0$parlist, an=0, z = data$Z, parallel = FALSE, nbtsp = 199, ninits = 10, eps=0.05)$crit
    crit.l <- NormalRegPanelMixture::regpanelmixCritBoot(y = data$Y, x = data$X, parlist = out.h0$parlist, an = 0, z = data$Z, parallel = FALSE, nbtsp = 199, ninits = 10, eps=0.01)$crit
    crit.h <- NormalRegPanelMixture::regpanelmixCritBoot(y = data$Y, x = data$X, parlist = out.h0$parlist, an = 0, z = data$Z, parallel = FALSE, nbtsp = 199, ninits = 10, eps=0.5)$crit
...
write.csv(rbind(result.h,result.m,result.l), file="results/sizeTestM2Regressorm_bound_boot.csv")
