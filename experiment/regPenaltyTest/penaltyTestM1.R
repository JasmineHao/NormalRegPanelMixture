...
anset <- c(0.001,0.005, 0.01, 0.05,0.1,0.2, 0.3, 0.4)
...
phi.data <- list()
nset <- length(Nset) * length(Tset)  * length(anset)
...
for (N in Nset){
  for (T in Tset){
      for (an in anset){
          t <- Sys.time()
          phi = list(alpha = c(1),mu = c(0),sigma = c(1), gamma = gamma,
...
write.csv(regression.data,file="/home/haoyu/results/penaltyTestM1.csv",row.names=FALSE)
