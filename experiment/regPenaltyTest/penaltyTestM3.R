...
getEstimate <- function(Data,phi,nrep,an,m,parlist,cl){
  ...
  results <- foreach (k = 1:nrep)%dopar% {
    ...
    crit <- try(  crit <- NormalRegPanelMixture::regpanelmixCrit(y=data$Y, x=data$X, parlist=parlist, z = data$Z, parallel = FALSE, nrep=1000)$crit)
      ...
      for (alpha in alphaset){
        for (sigma in sigmaset){
          for (an in anset){
            ...
            regression.data[count, ] <- cbind(result$nominal.size,an, phi$N,phi$T,omega)
            print(Sys.time() - t)
